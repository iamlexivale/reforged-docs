...
- Update for some reaspon all player data using luckperms as uuid and no longer using auth.
- Change `registered` to `nationBoard` in Nation List.
- Change `lastlogin` to `primary_group` in Player List.
- Change `lastlogin` to `primary_group` in Player Profile.
- Change `lastlogin` to `primary_group` in Player Statistics.
